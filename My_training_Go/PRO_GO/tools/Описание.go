...
Аргументами функции delete являются карта и ключ для удаления. Об ошибке не будет сообщено, если указанный ключ не
содержится в карте.
TODO Перечисление содержимого карты
Карты перечисляются с использованием ключевых слов for и range.
Когда ключевые слова for и range используются с картой, двум переменным присваиваются ключи и значения по мере перечис-
ления содержимого карты. (хотя они могут отображаться в другом порядке, как я объясню в следующем разделе).
TODO Перечисление карты по порядку
Вы можете увидеть результаты в другом порядке, потому что нет никаких гарантий, что содержимое карты будет пронумеровано
в каком-либо конкретном порядке. Если вы хотите получить значения на карте по порядку, то лучший подход — перечислить
карту и создать срез, содержащий ключи, отсортировать срез, а затем пронумеровать срез для чтения значений с карты.
TODO Понимание двойной природы строк
В главе 4 я описал строки как последовательности символов. Это правда, но есть сложности, потому что строки Go имеют
раздвоение личности в зависимости от того, как вы их используете.
Go обрабатывает строки как массивы байтов и поддерживает нотацию индекса массива и диапазона среза.
TODO Преобразование строки в руны
Тип rune представляет собой кодовую точку Unicode, которая по сути является одним символом. Чтобы избежать нарезки строк
в середине символов, можно выполнить явное преобразование в срез рун.
TODO Подсказка
...
TODO ПОНИМАНИЕ ПОЧЕМУ И БАЙТЫ, И РУНЫ ПОЛЕЗНЫ
Подход, который Go использует для строк, может показаться странным, но у него есть свое применение. Байты важны, когда
вы заботитесь о хранении строк, и вам нужно знать, сколько места нужно выделить. Символы важны, когда вы имеете дело с
содержимым строк, например, при вставке нового символа в существующую строку.
 Обе грани строк важны. Однако важно понимать, нужно ли вам иметь дело с байтами или символами для той или иной операции.
 У вас может возникнуть соблазн работать только с байтами, что будет работать до тех пор, пока вы используете только те
символы, которые представлены одним байтом, что обычно означает ASCII. Сначала это может сработать, но почти всегда
заканчивается плохо, особенно когда ваш код обрабатывает символы, введенные пользователем с набором символов, отличным
от ASCII, или обрабатывает файл, содержащий данные, отличные от ASCII. Для небольшого объема дополнительной работы проще
и безопаснее признать, что Unicode действительно существует, и полагаться на Go для преобразования байтов в символы.
TODO Перечисление строк
Цикл for можно использовать для перечисления содержимого строки. Эта функция показывает некоторые умные аспекты того,
как Go работает с отображением байтов в руны.
TODO Резюме
В этой главе я описал типы коллекций Go. Я объяснил, что массивы — это последовательности значений фиксированной длины,
срезы — это последовательности переменной длины, поддерживаемые массивом, а карты — это наборы пар ключ-значение.
Я продемонстрировал использование диапазонов для выбора элементов, объяснил связи между срезами и лежащими в их основе
массивами и показал, как выполнять распространенные задачи, такие как удаление элемента из
среза, для которых нет встроенных функций. Я закончил эту главу, объяснив сложную природу строк, которая может вызвать
проблемы упрограммистов, которые предполагают, что все символы могут быть представлены с помощью одного байта данных.
В следующей главе я объясню использование функций в Go.


TODO 8. Определение и использование функций

В этой главе я описываю функции Go, которые позволяют группировать операторы кода и выполнять их по мере необходимости. Функции Go
...
с которой сталкиваются функции.
TODO Определение простой функции
Функции — это группы операторов, которые можно использовать и повторно использовать как одно действие.
TODO Определение и использование параметров функции
Параметры позволяют функции получать значения данных при ее вызове, то позволяет изменить ее поведение.(str.261)
Примечание
Go не поддерживает необязательные параметры или значения по умолчанию для параметров.
TODO Пропуск типов параметров
Тип можно не указывать, если смежные параметры имеют одинаковый тип.
TODO Пропуск имен параметров
Символ подчеркивания (символ _) может использоваться для
параметров, определенных функцией, но не используемых в операторах кода функции.
TODO Определение вариационных параметров (str.265)
Вариативный параметр принимает переменное количество значений, что может упростить использование функций.
Чтобы понять проблему, которую решают вариативные параметры, полезно рассмотреть альтернативу.
